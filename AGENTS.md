...
- JavaScript for modern browsers; Node.js 24 for build and tests + `pdfjs-dist` for PDF parsing/rendering; `tesseract.js` plus vendored English traineddata for local OCR (005-ocr-scanned-ingestion)
- In-memory browser state plus browser-local IndexedDB cache for the vendored OCR language asset (005-ocr-scanned-ingestion)
...
## Recent Changes
- 005-ocr-scanned-ingestion: Added JavaScript for modern browsers; Node.js 24 for build and tests + `pdfjs-dist` for PDF parsing/rendering; `tesseract.js` plus vendored English traineddata for local OCR
