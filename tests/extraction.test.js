...
test("OCR fallback returns manual preconversion guidance in the single-file build", async () => {
  ...
  assert.match(result.message, /browser build/i);
});

test("OCR fallback returns recognized text when a local worker succeeds", async () => {
  const result = await ocrPipeline.attemptOcrFallback(
    {
      documentText: "",
      pages: [
        { pageNumber: 1, imageDataUrl: "data:image/png;base64,AAA" },
        { pageNumber: 2, imageDataUrl: "data:image/png;base64,BBB" }
      ]
    },
    {
      async createWorker() {
        return {
          async recognize(imageDataUrl) {
            return {
              data: {
                text: imageDataUrl.endsWith("AAA") ? "Name of plan\nNorthwind Pension Plan" : "Plan number 010",
                confidence: imageDataUrl.endsWith("AAA") ? 88.2 : 91.8
              }
            };
          },
          async terminate() {}
  ...
  assert.match(result.text, /Northwind Pension Plan/);
  assert.match(result.text, /Plan number 010/);
  assert.equal(result.pages.length, 2);
  assert.equal(result.confidence, 90);
});

test("OCR fallback preserves manual-review status when OCR recovers no text", async () => {
  const result = await ocrPipeline.attemptOcrFallback(
    {
      documentText: "",
      pages: [{ pageNumber: 1, imageDataUrl: "data:image/png;base64,AAA" }]
    },
    {
      async createWorker() {
        return {
          async recognize() {
            return { data: { text: "   ", confidence: 12 } };
          },
          async terminate() {}
        };
      }
  ...
  assert.equal(result.status, "ocr-manual-review");
  assert.match(result.message, /could not recover enough readable text/i);
});
