(function (global, factory) {
  ...
  function mapDocumentFields(input) {
    ...
    rawValues.planName = findTextValue(prepared.lines, ["name of plan", "plan name"])?.value || null;
    rawValues.planNumber =
      findSingleValue(prepared.joinedText, ["plan number", "\\bpn\\b"], "[A-Z0-9-]{1,12}")?.value || null;
    rawValues.sponsorEmployerIdentificationNumber =
      findSingleValue(prepared.joinedText, ["employer identification number", "\\bein\\b"], "\\d{2}-?\\d{7}")?.value ||
      null;
    rawValues.sponsorName = findTextValue(prepared.lines, ["name of plan sponsor", "plan sponsor"])?.value || null;
    rawValues.sponsorAddress = findTextValue(prepared.lines, ["mailing address", "address of plan sponsor"])?.value || null;
    rawValues.businessCode =
      findSingleValue(prepared.joinedText, ["business code", "principal business activity code"], "\\d{6}")?.value || null;
    rawValues.participantCountBeginningOfYear =
      findSingleValue(prepared.joinedText, ["participants at the beginning of the plan year", "line 6a"], "\\d[\\d,]*")?.value ||
      null;
    rawValues.retiredParticipantsReceivingBenefits =
      findSingleValue(prepared.joinedText, ["retired or separated participants receiving benefits", "line 6d"], "\\d[\\d,]*")?.value ||
      null;
    rawValues.separatedParticipantsEntitledToBenefits =
      findSingleValue(
        prepared.joinedText,
        ["separated participants entitled to future benefits", "line 6e"],
        "\\d[\\d,]*"
      )?.value || null;
    rawValues.deceasedParticipantsBeneficiaries =
      findSingleValue(
        prepared.joinedText,
        ["deceased participants whose beneficiaries are receiving or are entitled to receive benefits", "line 6f"],
        "\\d[\\d,]*"
      )?.value || null;
    rawValues.participantCountTotal =
      findSingleValue(prepared.joinedText, ["total number of participants", "line 6g"], "\\d[\\d,]*")?.value || null;
    rawValues.assetsBeginningOfYear = findNumberFromLinePair(prepared.lines, ["total assets", "line 1a"], 0)?.value || null;
    rawValues.assetsEndOfYear = findNumberFromLinePair(prepared.lines, ["total assets", "line 1b"], 1)?.value || null;
    rawValues.liabilitiesBeginningOfYear =
      findNumberFromLinePair(prepared.lines, ["total liabilities", "line 2a"], 0)?.value || null;
    rawValues.liabilitiesEndOfYear =
      findNumberFromLinePair(prepared.lines, ["total liabilities", "line 2b"], 1)?.value || null;
    rawValues.netAssetsBeginningOfYear =
      findNumberFromLinePair(prepared.lines, ["net assets", "line 3a"], 0)?.value || null;
    rawValues.netAssetsEndOfYear = findNumberFromLinePair(prepared.lines, ["net assets", "line 3b"], 1)?.value || null;
    rawValues.scheduleHAccountantOpinion =
      findTextValue(prepared.lines, ["accountant's opinion", "opinion"])?.value || null;
    rawValues.fundingTargetAttainmentPercent =
      findSingleValue(prepared.joinedText, ["funding target attainment percentage", "line 27"], "\\d[\\d.,]*%?")?.value ||
      null;
